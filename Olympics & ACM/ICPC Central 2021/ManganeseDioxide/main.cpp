...
Polynomial solve(int l, int r) {
    if (l == r) {
        return {1, (MOD - a[l]) % MOD};
    }
    int mid = (l + r) / 2;
    return multiply(solve(l, mid), solve(mid + 1, r), MOD);
}
...
int main(){  
    ...
    Polynomial prod = solve(1, n);
    Polynomial lnPoly = ln(prod, MOD, k);

    vector<int> res(k + 1);
    res[0] = n;
    for (int i = 1; i <= k; i++) {
        res[i] = (MOD - lnPoly[i]) * 1ll * i % MOD;
        cout << res[i] << "\n";
    }
